...
app.post('/api/users/login', (req, res) => {
    ...
    User.findOne({ email }, (err, userInfo) => {
        if(!userInfo) {
            return res.json({ success: false, message: "가입되지 않은 이메일입니다.", err })
        } else {
            //2. 비밀번호가 맞는지 확인
            userInfo.checkPassword(password, (err, isMatch) => {
                if(!isMatch) {
                    return res.json({ success: false, message: "올바르지 않은 비밀번호입니다.", err })
                } else {
                    //3. 토큰 생성
                    userInfo.generateToken((err, userInfo) => {
                        if(err) {
                            ...
                            .status(200).json({ success: true, userId: userInfo._id })
                        }
                    })
                }   
            })            
